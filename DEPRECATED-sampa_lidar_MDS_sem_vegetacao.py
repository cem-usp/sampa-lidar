...
def pipeline(scm, ano):
    ...
    pipeline = [
        ...
        {
            "filename":f"results/{ano}/MDS-sem-veg/MDS-sem-veg-{scm}-{ano}.tiff",
            "type": "writers.raster",
...
def processo(scm):
    ...
    if len(glob.glob(f"{RESULT_FOLDER}/2017/MDS-sem-veg/MDS-sem-veg-{scm}-2017.tiff")) > 0 and len(glob.glob(f"{RESULT_FOLDER}/2020/MDS-sem-veg/MDS-sem-veg-{scm}-2020.tiff")) > 0:
